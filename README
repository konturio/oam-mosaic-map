...
- `BASE_URL` is root url of the server.
- `TITILER_BASE_URL` is url of your TiTitiler installation.
- `TILES_CACHE_DIR_PATH` is path for tiles cache.
- `PGDATABASE`, `PORT`, `PGHOST`, `PGUSER`, `PGPASSWORD` PostgreSQL related variables.
...
## How does mosaic tiler works:

When the Mosaic Tiler receives a request for a tile, it handles it in the following steps:
1. Query the list of geotiffs contained within the requested tile from the database. Using PostgreSQL with PostGIS allows to delegate handling of spatial queries from the tiler to PostGIS.
2. Process each of the contained images, either by requesting TiTiler to generate a tile for a geotiff or by retrieving a tile from cache. It is important to note that the Mosaic Tiler does not handle geotiffs directly and instead delegates this responsibility entirely to TiTiler, focusing only on working with PNG and JPEG tile images.
